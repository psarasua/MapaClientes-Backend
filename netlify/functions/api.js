...
// Middleware personalizado para manejar Buffers en Netlify Functions
app.use((req, res, next) => {
  if (req.body && typeof req.body === 'object' && req.body.type === 'Buffer' && req.body.data) {
    try {
      const bufferData = Buffer.from(req.body.data);
      const jsonString = bufferData.toString('utf8');
      req.body = JSON.parse(jsonString);
      console.log('✅ Buffer parseado correctamente:', req.body);
    } catch (error) {
      console.error('❌ Error parseando Buffer:', error);
    }
  }
  next();
});
